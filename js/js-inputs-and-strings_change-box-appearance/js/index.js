...
const rotateRange = document.querySelector('[data-js="input-rotation"]');

const changeElement = document.querySelector('[data-js="box"]');

colorRange.addEventListener("input", () => {
  //   const red = colorRange.value;
  //   const green = 255 - colorRange.value;
  //   const blue = 128;
  changeElement.style.backgroundColor = `hsl(${
    colorRange.value - 1
  }, 100%, 50%)`;
});

radiusRange.addEventListener("input", () => {
  changeElement.style.borderRadius = `${radiusRange.value}%`;
});

rotateRange.addEventListener("input", () => {
  changeElement.style.transform = `rotate(${rotateRange.value}deg)`;
});
